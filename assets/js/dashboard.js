(function($) {
	...
	$(function() {
		...
		fetch('http://airobserver4-001-site1.htempurl.com/APIofSensors/aqiofday.php')
		.then(response => response.json())
		.then(data => {
		  // Update the HTML elements with the JSON data
		  const cardId = 'card-aqi'; // Change this to the data-card-id of the card you want to target
		  const card = document.querySelector(`[data-card-id="${cardId}"]`);
		  const h1Element = card.querySelector('.card-body h1');
		  const category = card.querySelectorAll('.card-body .text-secondary')[1];
		  const range = card.querySelectorAll('.card-body .text-secondary')[2];
		  h1Element.textContent = data.data.MAX;
		  category.textContent = data.data.Category;

	    // Get the aqi value
        const aqiValue = h1Element.innerText;
		// Select the progress bar and text-danger elements
		const progressBar = card.querySelector('.card-body .progress-bar');
		const textDangerNum = card.querySelectorAll('.card-body .text-danger-normal')[0];
		const textDanger = card.querySelectorAll('.card-body .text-danger-normal')[1];

		// Change the progress bar color and width
		if (aqiValue <= 50) {
			progressBar.classList.remove('bg-danger', 'bg-warning', 'bg-primary');
			progressBar.classList.add('bg-success');
			progressBar.style.width = aqiValue / 5 + '%'; // width will be 0-10%
			textDangerNum.style.color = '#28a745'; // set text color to green
			textDanger.style.color = '#28a745'; // set text color to green
			range.textContent = "0-50"
		  } else if (aqiValue <= 100) {
			progressBar.classList.remove('bg-success', 'bg-warning', 'bg-primary');
			progressBar.classList.add('bg-primary');
			progressBar.style.width = (aqiValue - 50) / 5 + 10 + '%'; // width will be 10-30%
			textDangerNum.style.color = '#007bff'; // set text color to blue
			textDanger.style.color = '#007bff'; // set text color to blue
			range.textContent = "51-100"
		  } else if (aqiValue <= 150) {
			progressBar.classList.remove('bg-success', 'bg-primary', 'bg-warning');
			progressBar.classList.add('bg-warning');
			progressBar.style.width = (aqiValue - 100) / 5 + 30 + '%'; // width will be 30-50%
			textDangerNum.style.color = '#ffc107'; // set text color to yellow
			textDanger.style.color = '#ffc107'; // set text color to yellow
			range.textContent = "101-150"
		  } else if (aqiValue <= 200) {
			progressBar.classList.remove('bg-success', 'bg-primary', 'bg-warning');
			progressBar.classList.add('bg-danger');
			progressBar.style.width = (aqiValue - 150) / 5 + 50 + '%'; // width will be 50-70%
			textDangerNum.style.color = '#dc3545'; // set text color to red
			textDanger.style.color = '#dc3545'; // set text color to red
			range.textContent = "151-200"
		  } else if (aqiValue <= 300) {
			progressBar.classList.remove('bg-success', 'bg-primary', 'bg-danger');
			progressBar.classList.add('bg-dark');
			progressBar.style.width = (aqiValue - 200) / 10 + 70 + '%'; // width will be 70-80%
			textDangerNum.style.color = '#6c757d'; // set text color to grey
			textDanger.style.color = '#6c757d'; // set text color to dark grey
			range.textContent = "201-300"
		  } else {
			progressBar.classList.remove('bg-success', 'bg-primary', 'bg-dark');
			progressBar.classList.add('bg-black');
			progressBar.style.width = '80%'; // width will be 80%
			textDangerNum.style.color = '#343a40'; // set text color to black
			textDanger.style.color = '#343a40'; // set text color to black
			range.textContent = "301-500+"
		  }
		
		  if (aqiValue <= 50) {
			textDanger.innerHTML = '<i class="fas fa-check-circle"></i> Air quality is good.';
		  } else if (aqiValue <= 100) {
			textDanger.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Air quality is moderate.';
		  } else if (aqiValue <= 150) {
			textDanger.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Air quality is unhealthy for sensitive groups.';
		  } else if (aqiValue <= 200) {
			textDanger.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Air quality is unhealthy.';
		  } else if (aqiValue <= 300) {
			textDdanger.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Air quality is very unhealthy.';
		  } else {
		    textDanger.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Air quality is hazardous.';
		}
		...
		if ($(".dashboard-progress-1").length) {
			...
			// Make a request to the API using the fetch method
			fetch('http://airobserver4-001-site1.htempurl.com/APIofSensors/aqiOfDay.php')
			.then(response => response.json()) // Parse the response as JSON
			.then(data => {
			// Extract the values you need from the data
			const value1 = data.data.PM10;
			// Set the values in the h2 and h3 elements of the card with data-card-id="card-1"
			const cardId = 'card-1'; // Change this to the data-card-id of the card you want to target
			const card = document.querySelector(`[data-card-id="${cardId}"]`);
			const h2Element = card.querySelector('.card-body i');
			const h3Element = card.querySelector('.card-body h3');
			h2Element.textContent = value1+" ppm";
			h3Element.textContent = "50%";
			})
			.catch(error => {
			console.error(error);
			});
		...
		if ($(".dashboard-progress-2").length) {
			...
			.then(data => {
			// Extract the values you need from the data
			const value1 = data.data.PM25;
			// Set the values in the h2 and h3 elements of the card with data-card-id="card-1"
			const cardId = 'card-2'; // Change this to the data-card-id of the card you want to target
			const card = document.querySelector(`[data-card-id="${cardId}"]`);
			const h2Element = card.querySelector('.card-body i');
			const h3Element = card.querySelector('.card-body h3');
			h2Element.textContent = value1+" ppm";
		...
		if ($(".dashboard-progress-3").length) {
			...
			.then(data => {
			// Extract the values you need from the data
			const value1 = data.data.O3;
			// Set the values in the h2 and h3 elements of the card with data-card-id="card-1"
			const cardId = 'card-3'; // Change this to the data-card-id of the card you want to target
			const card = document.querySelector(`[data-card-id="${cardId}"]`);
			const h2Element = card.querySelector('.card-body i');
			const h3Element = card.querySelector('.card-body h3');
			h2Element.textContent = value1+" ppm";
		...
		if ($(".dashboard-progress-4").length) {
			...
			.then(data => {
			// Extract the values you need from the data
			const value1 = data.data.NO2;
			// Set the values in the h2 and h3 elements of the card with data-card-id="card-1"
			const cardId = 'card-4'; // Change this to the data-card-id of the card you want to target
			const card = document.querySelector(`[data-card-id="${cardId}"]`);
			const h2Element = card.querySelector('.card-body i');
			const h3Element = card.querySelector('.card-body h3');
			h2Element.textContent = value1+" ppm";
		...
		if ($(".dashboard-progress-4-dark").length) {
			...
			.then(data => {
			// Extract the values you need from the data
			const value1 = data.data.NO2+" ppm";
			// Set the values in the h2 and h3 elements of the card with data-card-id="card-1"
			const cardId = 'card-4'; // Change this to the data-card-id of the card you want to target
			const card = document.querySelector(`[data-card-id="${cardId}"]`);
			const h2Element = card.querySelector('.card-body i');
			const h3Element = card.querySelector('.card-body h3');
			h2Element.textContent = value1;
			h3Element.textContent = "50%";
			})
			.catch(error => {
			console.error(error);
	...
	if ($(".dashboard-progress-5").length) {
		$('.dashboard-progress-5').circleProgress({
			value: 0.45,
			size: 125,
			thickness: 7,
			startAngle: 10,
		...
		.then(data => {
		// Extract the values you need from the data
		const value1 = data.data.SO2;
		// Set the values in the h2 and h3 elements of the card with data-card-id="card-1"
		const cardId = 'card-5'; // Change this to the data-card-id of the card you want to target
		const card = document.querySelector(`[data-card-id="${cardId}"]`);
		const h2Element = card.querySelector('.card-body i');
		const h3Element = card.querySelector('.card-body h3');
		h2Element.textContent = value1+" ppm";
	...
	if ($(".dashboard-progress-5-dark").length) {
		$('.dashboard-progress-5-dark').circleProgress({
			value: 0.45,
			size: 125,
			thickness: 7,
			startAngle: 10,
...
if ($(".dashboard-progress-6").length) {
	$('.dashboard-progress-6').circleProgress({
		value: 0.45,
		size: 125,
		thickness: 7,
		startAngle: 10,
	...
	.then(data => {
	// Extract the values you need from the data
	const value1 = data.data.CO;
	// Set the values in the h2 and h3 elements of the card with data-card-id="card-1"
	const cardId = 'card-6'; // Change this to the data-card-id of the card you want to target
	const card = document.querySelector(`[data-card-id="${cardId}"]`);
	const h2Element = card.querySelector('.card-body i');
	const h3Element = card.querySelector('.card-body h3');
	h2Element.textContent = value1+" ppm";
...
if ($(".dashboard-progress-6-dark").length) {
	$('.dashboard-progress-6-dark').circleProgress({
		value: 0.45,
		size: 125,
		thickness: 7,
		startAngle: 10,
	...
	console.error(error);
		...
		if ($("#device-sales").length) {
			var deviceSalesData = {
				labels: ["Cairo", "Giza", "Alex", "Aswan", "Assiut", "Sohag", "Almonufia", "Luxor"],
				datasets: [{
						label: 'AQI',
						data: [450, 500, 300, 350, 200, 320, 310, 100],
						backgroundColor: [
							'#0062ff', '#0062ff', '#0062ff', '#0062ff', '#0062ff', '#0062ff', '#0062ff', '#0062ff',
						],
						borderColor: [
							'#0062ff', '#0062ff', '#0062ff', '#0062ff', '#0062ff', '#0062ff', '#0062ff', '#0062ff',
						],
		...
		if ($("#page-view-analytic").length) {
			...
			fetch('http://airobserver4-001-site1.htempurl.com/APIofSensors/aqiGraphLastHours.php')
			.then(response => response.json())
			.then(data => {
				console.log(data);
				// Extract the MAX values from the data array
				const maxValues = data.data.map(item => parseFloat(item.MAX));
			  
				// Update the pageiVewAnalyticData object with the new data
				pageiVewAnalyticData.datasets[0].data = maxValues;
